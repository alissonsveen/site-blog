    ...
    <section className="container py-8 md:py-10">
      <div className="flex flex-col items-center gap-12">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {costumerStories.map((costumerStory) => (
            <div
              key={costumerStory.author.name}
              className="flex flex-col gap-6 roundelg bg-gray-500 p-6 md:p-12"
            >
              <p className="text-balance text-gray-200">
                {costumerStory.content}
              </p>

              <div className="flex items-center gap-3">
                <div className="relative h-10 w-10 overflow-hidden rounded-full">
                  <Image
                    src={costumerStory.author.avatar}
                    alt={costumerStory.author.name}
                    fill
                ...
                <div className="flex flex-col">
                  <strong className="text-gray-200 text-sm">
                    {costumerStory.author.name}
                  </strong>
                  <span className="text-xs text-gray-300">
                    {costumerStory.author.role}
                  </span>
                </div>
              </div>
            </div>
